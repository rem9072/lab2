...
lab2mod<-lab2 %>%
  gather(2:10, key="time_category", value="score")%>% 
...
coverage <- read.csv("C:/Users/rfmag/Downloads/coverage.csv",skip=2,stringsAsFactors=FALSE,na.strings="N/A",header=TRUE)
expend <- read.csv("C:/Users/rfmag/Downloads/expenditures.csv",skip=2,stringsAsFactors=FALSE,na.strings="N/A",header=TRUE)

coveragemod<-coverage[-c(68:53),] %>%
  gather(2:29, key="year_coverage", value="number_enrollees")%>% 
